                    ...
                    @foreach($languages as $language)
                        <div class="tab-pane {{$language->locale==Lang::getLocale() ? 'active' : ''}}" id="tab_{{$language->id}}">
                            <div class="form-group">
                                ...
                                <div class="col-lg-10">
                                    <input type="text" name="heading[{{$language->locale}}]" value="{{ $ourStoryTransaltionsLite['heading'][$language->locale] }}" class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-lg-2">Текст</label>
                                <div class="col-lg-10">
                                    <textarea id="text[{{$language->locale}}]" name="text[{{$language->locale}}]" class="form-control">{!! $ourStoryTransaltionsLite['text'][$language->locale] !!}</textarea>
                                </div>
                            </div>
                        </div>
                    @endforeach
