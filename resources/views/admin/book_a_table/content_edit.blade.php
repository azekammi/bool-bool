<div class="panel panel-flat">
    <div class="panel-heading">
        <h6 class="panel-title">Заказать столик</h6>
    </div>
                    ...
                    @foreach($languages as $language)
                        <div class="tab-pane {{$language->locale==Lang::getLocale() ? 'active' : ''}}" id="tab_{{$language->id}}">
                            <div class="form-group">
                                ...
                                <div class="col-lg-10">
                                    <input type="text" name="heading[{{$language->locale}}]" value="{{ $bookATableTransaltionsLite['heading'][$language->locale] }}" class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-lg-2">Описание</label>
                                <div class="col-lg-10">
                                    <textarea id="description[{{$language->locale}}]" name="description[{{$language->locale}}]" class="form-control">{!! $bookATableTransaltionsLite['description'][$language->locale] !!}</textarea>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
@foreach($languages as $language)
    <script>
        CKEDITOR.replace('description[{{$language->locale}}]');
    </script>
@endforeach