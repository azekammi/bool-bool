    ...
    <div class="panel-body">
        <form method="post" class="form-horizontal">
                    ...
                    @foreach($languages as $language)
                        <div class="tab-pane {{$language->locale==Lang::getLocale() ? 'active' : ''}}" id="tab_{{$language->id}}">
                            <div class="form-group">
                                ...
                                <div class="col-lg-10">
                                    <input type="text" name="name[{{$language->locale}}]" value="{{ $menuDishTransaltionsLite['name'][$language->locale] }}" class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="control-label col-lg-2">Описание</label>
                                <div class="col-lg-10">
                                    <textarea name="description[{{$language->locale}}]" class="form-control">{{ $menuDishTransaltionsLite['description'][$language->locale] }}</textarea>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="form-group">
                ...
                <div class="col-lg-10">
                    <input type="text" name="price" value="{{ $menuDish->price/100 }}" class="form-control">
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-lg-10">
                    <select name="menu_category_id" class="form-control">
                        @foreach($menuCategories as $menuCategory)
                            <option @if($menuDish->menu_category_id == $menuCategory->id) {{"selected"}} @endif value="{{$menuCategory->id}}">{{$menuCategory->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
